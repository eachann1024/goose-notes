...
export function ComposerSuggestionsList({
  ...
  const estimatedHeight =
    items.length === 0
      ...
      : Math.min(
          POPOVER_MAX_HEIGHT,
          items.length * ITEM_HEIGHT + Math.max(0, items.length - 1) * ITEM_GAP + LIST_PADDING,
        );
  ...
  const maxHeight = Math.min(
    POPOVER_MAX_HEIGHT,
    Math.max(items.length === 0 ? POPOVER_EMPTY_HEIGHT : ITEM_HEIGHT + LIST_PADDING, available),
  );

  // 空 range / 异常 rect 时退回到视口安全区，避免 left/top 落到 0 把菜单甩到左上角
  const anchorLeft =
    Number.isFinite(anchorRect.left) && (anchorRect.width > 0 || anchorRect.height > 0)
      ? anchorRect.left
      : VIEWPORT_PADDING;
  const maxLeft = window.innerWidth - POPOVER_MAX_WIDTH - VIEWPORT_PADDING;
  ...
  const popover = (
    ...
    >
      {items.length === 0 ? (
        <div className="px-3 py-2.5 text-[13px] text-muted-foreground">未找到匹配笔记</div>
      ) : (
        <ul
          ref={listRef}
